...
PeerKitServer.prototype._getChunks = async.memoize(function(resource, cb) {
  ((resource.url.substring(0, 6) == 'https:') ? https : http).get(resource.url, function(res) {
    ...
    console.log('Chosen chunk size', chunkSize, 'for file size', len);
...
PeerKitServer.prototype._requestResource = function(url) {
  util.log("Resource requested:", url, this._resources[url]);
  ...
  if (clients.length <= this._options.multiplicity) {
    ...
    while (conns.length < this._options.multiplicity) {
      rand = Math.floor(Math.random() * clients.length);
      if (!seen[rand]) {
