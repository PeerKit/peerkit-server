...
PeerKitServer.prototype._initializeWSS = function(){
  ...
  var x = [];
  this._wss2 = new WebSocketServer({ path: '/ws2' , port: 9002});
  this._wss2.on('connection', function(socket) {
    x.push(socket);
  });
  
    var sendNote = function(f, d){
    console.log('sending note', f, d);
      rest.get('http://freegeoip.net/json/'+f).on('complete', function(r1) {
        if (r1 instanceof Error) {
          
        } else {
         rest.get('http://freegeoip.net/json/'+d).on('complete', function(r2) {
            if (r2 instanceof Error) {
              
            } else {
              for (var i = 0; i < x.length; i++) {
                  x[i].send(JSON.stringify([ r1, r2 ]));
                };
            }
          }); 
        }
    ...
    socket.on('message', function(data){
      ...
      switch (message.type) {
        case 'RESPONSE':
          ...
          //
          if(self._clients[message.id]) {
            sendNote(client.ip, self._clients[message.id].ip);
          }
