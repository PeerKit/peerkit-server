...
function PeerCdnServer(options) {
  if (!(this instanceof PeerCdnServer)) return new PeerCdnServer(options);
...
util.inherits(PeerCdnServer, EventEmitter);

PeerCdnServer.VALID_HOSTNAMES = {'www.google.com': true, 'demo.peerkit.com': true};

PeerCdnServer.prototype._initializeWSS = function(){
...
PeerCdnServer.prototype._killClient = function(id){
...
PeerCdnServer.prototype._initializeHTTP = function(){
  var self = this;
 ...
 */
PeerCdnServer.prototype._reportResource = function(client, resource) {
...
PeerCdnServer.prototype._getChunks = async.memoize(function(resource, cb) {
  var parsedUrl = url.parse(resource.url);
  if (!PeerCdnServer.VALID_HOSTNAMES[parsedUrl.hostname]) {
 ...
 */
PeerCdnServer.prototype._requestResource = function(url) {
...
PeerCdnServer.prototype._requestConns = function(id, count) {
  this._clients[id].send({type: 'REPLENISH', count: count});
};

PeerCdnServer.prototype._forward = function(dest, data) {
...
PeerCdnServer.prototype._generateId = function(key) {
